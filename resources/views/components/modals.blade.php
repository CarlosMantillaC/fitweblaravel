...
<div x-show="{{ $createShowVar }}" x-transition:enter="transition ease-out duration-300"
    ...
    <div class="bg-[#151515] w-full max-w-2xl rounded-lg shadow-xl overflow-hidden">
        <div class="flex justify-between items-center p-4 border-b border-gray-700">
            <h2 class="text-xl font-bold text-tertiary">{{ $createTitle }}</h2>
            <button @click="{{ $createShowVar }} = false" class="text-gray-400 hover:text-white">
...
<div x-show="{{ $editShowVar }}" x-transition:enter="transition ease-out duration-300"
    ...
    <div class="bg-[#151515] w-full max-w-3xl rounded-lg shadow-xl overflow-hidden">
        <div class="flex justify-between items-center p-4 border-b border-gray-700">
            <h2 class="text-xl font-bold text-tertiary">{{ $editTitle }}</h2>
            <button @click="{{ $editShowVar }} = false" class="text-gray-400 hover:text-white">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M6 18L18 6M6 6l12 12" />
                </svg>
