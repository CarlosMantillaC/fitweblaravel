@if ($paginator->hasPages())
    <nav role="navigation" class="flex items-center justify-between" aria-label="Pagination Navigation">
        ...
        <div class="inline-flex space-x-1 rounded-xl bg-[#0A0A0A] px-4 py-2 border border-transparent">
                ...
                @else
                    <a href="{{ $paginator->previousPageUrl() }}" class="px-3 py-1 text-sm text-white bg-[#151515] hover:bg-[#f36100] rounded">Anterior</a>
                @endif
                ...
                @else
                    <span class="px-3 py-1 text-sm text-gray-500 bg-[#151515] rounded">Siguiente</span>
                @endif
            </div>
        </div>
    </nav>
